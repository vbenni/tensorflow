...
std::shared_ptr<ov::Node> Add::createNode() {
    ...
    auto inputNode1 = getInputNode(tensor_indices[0]);
    if (inputNode1 == nullptr) TFLITE_LOG(INFO) << "input node 1 is null\n";
    auto inputNode2 = getInputNode(tensor_indices[1]);
    if (inputNode2 == nullptr) TFLITE_LOG(INFO) << "input Node 2 is null\n";
    auto addNode =
